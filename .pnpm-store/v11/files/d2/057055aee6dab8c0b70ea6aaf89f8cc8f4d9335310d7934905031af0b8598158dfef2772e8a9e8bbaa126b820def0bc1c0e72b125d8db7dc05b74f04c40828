...
function colorResolver(property, varName, key, shouldPass) {
  return ([, body], { theme, generator }) => {
    ...
    if (cssColor) {
      if (alpha != null) {
        css[property] = ruleUtils.colorToString(cssColor, alpha) + rawColorComment;
      } else {
      ...
      if (alpha != null) {
        css[property] = ruleUtils.colorToString(color, alpha) + rawColorComment;
      } else {
        const opacityVar = `--un-${varName}-opacity`;
...
function colorableShadows(shadows, colorVar) {
  ...
  for (let i = 0; i < shadows.length; i++) {
    ...
    } else if (ruleUtils.parseCssColor(lastComp)) {

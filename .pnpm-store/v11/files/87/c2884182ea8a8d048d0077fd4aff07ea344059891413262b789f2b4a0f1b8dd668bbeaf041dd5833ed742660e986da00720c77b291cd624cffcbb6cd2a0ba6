...
import { SFCParseOptions, SFCDescriptor, SFCBlock } from '@vue/compiler-sfc';
...
declare class Page {
    ...
    getPageMeta(forceUpdate?: boolean): Promise<PageMetaDatum>;
    getTabBar(forceUpdate?: boolean): Promise<TabBarItem & {
        index: number;
    } | undefined>;
    hasChanged(): boolean;
    read(): Promise<void>;
    private readPageMetaFromFile;
}
declare function parseSFC(code: string, options?: SFCParseOptions): SFCDescriptor;
declare function tryPageMetaFromMacro(sfc: SFCDescriptor): Promise<UserPageMeta | undefined>;
declare function tryPageMetaFromCustomBlock(sfc: SFCDescriptor, routeBlockLang: RouteBlockLang): Promise<UserPageMeta>;
declare function findMacro(stmts: t.Statement[], filename: string): t.CallExpression | undefined;
declare function findImports(stmts: t.Statement[]): t.ImportDeclaration[];
...
declare class PageContext {
    ...
    subPages: Map<string, Map<string, Page>>;
    pageMetaData: PageMetaDatum[];
    subPageMetaData: SubPageMetaDatum[];
    resolvedPagesJSONPath: string;
    private resolvedPagesJSONIndent?;
    private resolvedPagesJSONNewline?;
    private resolvedPagesJSONEofNewline?;
     ...
     * @returns pages rules
     */
    parsePages(pages: Map<string, Page>, type: 'main' | 'sub', overrides?: PageMetaDatum[]): Promise<PageMetaDatum[]>;
    /**
     * set home page
    ...
    private genratePagesJSON;
    private readInfoFromPagesJSON;
    private getIndent;
    private getNewline;
 ...
 */
declare function parseCode(options: {
    imports: string[];
    code: string;
    filename: string;
}): Promise<any>;
declare function sleep(ms: number): Promise<unknown>;
declare const babelGenerate: typeof babelGenerator;
...
type RouteBlockLang = 'json5' | 'jsonc' | 'json' | 'yaml' | 'yml';
interface Options {
    /**
...
interface UserTabBarItem extends Partial<TabBarItem> {
    /**
     * 配置页面路径
...
interface UserPageMeta extends Partial<PageMetaDatum> {
    /**
     * 配置页面路径
     * @deprecated 可选，将会根据文件路径自动生成
     */
...
type MaybePromiseCallable<T> = T | (() => T) | (() => Promise<T>);
declare function definePage(options: MaybePromiseCallable<UserPageMeta>): void;
type DefinePage = typeof definePage;
...
declare function parseCustomBlock(block: SFCBlock, filePath: string, routeBlockLang: RouteBlockLang): CustomBlock | undefined;
declare function getRouteSfcBlock(sfc?: SFCDescriptor): SFCBlock | undefined;
declare function getRouteBlock(path: string, blockStr: SFCBlock | undefined, routeBlockLang: RouteBlockLang): CustomBlock | undefined;
 ...
 */
declare function checkPagesJsonFileSync(path: fs.PathLike): boolean;
declare function writeFileWithLock(path: string, content: string, retry?: number): Promise<void>;

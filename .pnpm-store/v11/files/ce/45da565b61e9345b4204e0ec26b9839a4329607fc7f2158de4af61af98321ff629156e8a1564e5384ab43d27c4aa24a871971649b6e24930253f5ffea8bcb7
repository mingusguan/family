 ...
 * @returns
 */
declare function isUndef(v: unknown): v is null | undefined;
/**
 * 是否为空字符串
 */
 ...
 */
declare function obj2Url(obj: Record<string, any>, encode?: boolean): string;
/**
 * 将总秒数转换成 时:分:秒
 * @param seconds - 秒
 ...
 */
declare function fillZero(num: number): string;
/**
 * 转柯里化函数
 * @param fn - 函数
 * @example
...
declare class Updater {
    oldScript: string[];
    newScript: string[];
    dispatch: Record<string, AnyFn[]>;
    constructor(options: Options);
    init(): Promise<void>;
    getHtml(): Promise<string>;
    parserScript(html: string): string[];
    on(key: 'no-update' | 'update', fn: AnyFn): this;
    compare(oldArr: string[], newArr: string[]): void;
    timing(time?: number): void;
}

export { DATE_FORMAT, HTTP_CONTENT_TYPE, HTTP_REQUEST, HTTP_STATUS, OSType, TYPE_OF, Updater, addDays, all, allEqual, camelize, capitalize, clone, closest, compareVersion, createArray, debounce, download, downloadFile, drawImageVerify, escapeHTML, escapeRegExp, fillZero, formatDate, getByPath, getDayByNumber, getDaysOfLastMonth, getDaysOfMonth, getDaysOfWeek, getDaysToNowOfMonth, getFirstDayOfMonth, getFirstDayOfYear, getHanByNumber, getLastDayOfMonth, getNow, getOSType, goTo, goToTop, groupArrayByKey, hasChanged, hasOwn, hexToRGB, hideMobile, hyphenate, indexOfMax, indexOfMin, insertAt, insertStr, intersection, invokeArrayAsyncFns, invokeArrayFns, isAfter, isArray, isBefore, isBetween, isDate, isEmpty, isEmptyString, isFile, isFunction, isMap, isMobile, isMobileAgent, isNull, isNumber, isObject, isPlainObject, isPromise, isRegExp, isSet, isString, isSymbol, isUndef, isUndefined, last, lastN, loadJs, nanoid, normalize, obj2Url, objectToString, omit, omitBy, pick, pickBy, randomBoolean, randomHexColorCode, randomIntegerInRange, randomNumberInRange, randomRgbColor, reload, removeAt, removeByProp, removeDuplicates, removeWhitespace, replaceAll, required, rgbToHex, scrollToBottom, scrollToTop, seconds2DayTime, seconds2Time, shuffleArray, sleep, stringHash, subDays, throttle, toCamelCase, toCharArray, toCurryFunc, toDate, toHex, toKebabCase, toNumber, toPascalCase, toSnakeCase, toTypeString, transpose, trim, trimAll, uuid };

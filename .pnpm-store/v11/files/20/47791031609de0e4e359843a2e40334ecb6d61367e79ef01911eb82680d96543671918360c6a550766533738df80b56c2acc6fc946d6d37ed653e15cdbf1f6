...
require('./shared/unocss-preset-weapp.9b4df821.cjs');
require('./utils.cjs');
  ...
  return [
    {
      ...
      getCSS(ctx) {
        if (ctx.theme.preflightBase) {
          ...
          const roots = core.toArray(preflightRoot);
...
const presetWeapp = core.definePreset((options = {}) => {
  ...
  return {
    ...
    postprocess(css) {
      if (options.transform)
        css.selector = utils.cacheTransformEscapESelector(css.selector, options.transformRules);

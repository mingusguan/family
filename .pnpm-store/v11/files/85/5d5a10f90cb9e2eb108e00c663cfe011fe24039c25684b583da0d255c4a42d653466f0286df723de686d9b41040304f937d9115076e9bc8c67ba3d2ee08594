...
function taggedAria(tagName) {
  return {
    name: `${tagName}-aria`,
    match(matcher, ctx) {
      const variant = ruleUtils.variantGetParameter(`${tagName}-aria-`, matcher, ctx.generator.config.separators);
      if (variant) {
        const [match, rest] = variant;
        const ariaAttribute = utilities.h.bracket(match) ?? ctx.theme.aria?.[match] ?? "";
        if (ariaAttribute) {
          return {
...
const variantChildren = [
  utils.variantMatcher("*", (input) => ({ selector: `${input.selector} > *` }))
];
...
function negateMathFunction(value) {
  const match = value.match(utilities.cssMathFnRE) || value.match(utilities.cssVarFnRE);
  if (match) {
...
exports.variantStartingStyle = variantStartingStyle;
exports.variantSupports = variantSupports;
exports.variantTaggedAriaAttributes = variantTaggedAriaAttributes;

    ...
    <view class="profile-content__card-area">
      <wd-card v-if="userProfile">
        <wd-cell-group border>
            ...
            <view class="avatar-cell__avatar">
              ...
              >
                <image
                  ...
                  mode="aspectFill"
                />
                <view v-else class="avatar-cell__img">
          ...
          <wd-cell
            title="昵称"
            :value="userProfile.nickname || '未设置'"
            is-link
            @click="openDialog"
          />
...
async function saveAvatar(path: string) {
...
function handleChooseAvatar(event: { detail: { avatarUrl: string } }) {
  return saveAvatar(event.detail.avatarUrl);
}

function handleAvatarConfirm(event: { tempFilePath: string }) {
  return saveAvatar(event.tempFilePath);
}
